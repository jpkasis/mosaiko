 ...
 * reintroduce R2 references or unsafe production cache fallbacks:
 *
 ...
 *   - A1: `src/lib/cart-composite-blob-cache.ts` uses a Vercel-writable
 *     production cache dir without an ungated env-derived trace root
...
describe('UAT-3 Phase 4 — residue removed (Codex audit cleanup)', () => {
  ...
  test('A1: cart-composite blob cache keeps production cache path trace-stable', () => {
    const src = read('src/lib/cart-composite-blob-cache.ts');

    expect(src).toMatch(/process\.env\.NODE_ENV === ['"]production['"]/);
    expect(src).toMatch(
      /path\.join\(\s*['"]\/tmp['"]\s*,\s*['"]mosaiko-cart-composite-cache['"]\s*\)/,
    );
    expect(src).toMatch(
      /process\.env\.NODE_ENV !== ['"]production['"]\s*&&\s*process\.env\.CART_COMPOSITE_CACHE_DIR/,
    );

    // The deploy-breaking shape was an ungated production env override:
    // `const CACHE_DIR = process.env.CART_COMPOSITE_CACHE_DIR ? ...`.
    // That makes Next/Vercel's NFT tracer conservatively include broad
    // project assets in /api/cart-composite.
    expect(src).not.toMatch(
      /const\s+CACHE_DIR\s*=\s*process\.env\.CART_COMPOSITE_CACHE_DIR\s*\?/,
    );
  });
