...
/**
 * Contenido tab — edits the home-copy metaobject (ES + EN). Logic moved
 * verbatim from the PR3 ConfiguracionContent; behavior unchanged.
 */
export function ContenidoPanel({ fallbacks }: ContenidoPanelProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (cancelled) return;
        console.error('[ContenidoPanel] load failed:', error);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      console.error('[ContenidoPanel] save failed:', error);
    ...
    <div className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-warm-gray">
          ...
          mostrarán el texto predeterminado del sitio (visible como marcador en
          cada campo).
        </p>
      </header>
